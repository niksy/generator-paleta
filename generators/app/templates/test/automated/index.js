...
<% if ( esModules ) { %>import fn from '../<% if ( manualTests || integrationTests ) { %>../<% } %><% if ( complexTranspile ) { %>src/<% } %>index';<% } else { %><% if ( transpile ) { %>const<% } else { %>var<% } %> fn = require('../<% if ( manualTests || integrationTests ) { %>../<% } %><% if ( transpile ) { %><% if ( complexTranspile ) { %>src/<% } %>index<% } %>');<% } %><% if ( usesHtmlFixtures ) { %>
